...
<%
   ...
   Gson gson = new Gson();
	   ...
	   ArrayList<Code> food2s = new ArrayList<Code>();
	   Food food= cd.getFoodCode(fc);
	   Code food2= new Code("FoodCode","","","식재료명","","");
	   foods.add(food);
	   food2s.add(food2);
...
<body>
<div>
<span class="title">부식재고 목록 수정</span>
...
<caption>조회 목록</caption> 
    <tr>
      ...
      <td class="col" >
	  <input type="text" id ="Food" value="<%=food.getFoodName()%>">
   </tr>
   <tr>
      <td class="colt" >재고번호</td>
      <td class="col" >
	  <input type="text" id ="FoodCode" value="<%=food.getFoodCode()%>" disabled >
   </tr>
    <tr>
      <td class="colt" >단위</td>
      <td class="col" >
	  <input type="text" id ="Unit" value="<%=food.getUnit()%>" >
   </tr>
     <tr>
   ...
   <tr>
      ...
      <td class="col" ><input type="date" id="UpdateDate" value="<%=food.getUpdateDate()%>" disabled></td>
   </tr>
    <tr>
      <td class="colt" >비고</td>
      <td class="col" >
      <input type="text" id ="remark" value="<%=food.getRemark()%>">
      </td>
</tr>
</table>
<input type="button" id="edit" value="수정" onclick="pmUpdate()">
<input type="button" id="back" value="이전" onclick="location.href='foodCodeList.jsp'">
...
// setTimeout('go_url()',10000)  // 10초후 go_url() 함수를 호출
	$('#FoodSource').val('<%=food.getFoodSource()%>').prop("selected", true);	
	...
	$.ajax({
		...
		success: function(response) {
			// success handle
				...
				alert(data[0].foodName+"가(이) 수정되었습니다.");
				location.reload();
