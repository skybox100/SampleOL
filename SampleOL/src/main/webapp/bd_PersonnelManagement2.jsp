...
<%
   ...
   Gson gson = new Gson();
	...
	ArrayList<String> rcp_0 = cd.getMobileStatusRcId("28여단");
	ArrayList<String> rcp_1 = cd.getMobileStatusRcId("28-1대대");
	ArrayList<String> rcp_2 = cd.getMobileStatusRcId("28-2대대");
	ArrayList<String> rcp_3 = cd.getMobileStatusRcId("28-3대대");
	...
	if(personnelmanagements.get(0).getPicture() != "")
		Picture="data:image/png;base64, "+personnelmanagements.get(0).getPicture();
%>
...
<caption>조회 목록</caption> 
   <tr>
     ...
     <%
  		out.println("<img id=\"picture\" src=\""+Picture+"\" width=\"170px\"  style=\"margin-bottom: 4px; max-height:170px\" />");
  	  %>
   ...
   <tr>
	  ...
	  <td style="border-top:none;">
            ...
            <label class="btn btn-primary btn-file" id="fileAdd" style="display: block; borad:none;">
   	 파일추가 <input id="picturefile" type="file" accept="image/jpg,image/png,image/jpeg" style="display: none;" onchange="onFileSelected(event)">
		</label>
		 <label class="btn btn-primary btn-file" id="fileEdit" style="display: none;">
   	 수정 <input type="file" accept="image/jpg,image/png,image/jpeg" style="display: none;" onchange="onFileSelected(event)">
		</label>
	  </td>
...
$(document).ready(function() {
	...
	regSelectChange('<%=personnelmanagements.get(0).getRegimentName() %>');
 ...
 // setTimeout('go_url()',10000)  // 10초후 go_url() 함수를 호출
	...
	function onFileSelected(event) {
		  ...
		  data[0].Picture=$("input[id=picturefile]")[0].files[0];
		}
...
function pwReset(){
	document.getElementById("pw").value='1234567890';
	data[0].Password=$('#pw').val();
$.ajax({
